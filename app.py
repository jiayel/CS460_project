...
def register_user():
    ...
    if test:
        print(cursor.execute(
            "INSERT INTO Users (First_name,Last_name, email, password,Hometown,Gender, Date_of_birth) VALUES ('{0}', '{1}','{2}','{3}','{4}','{5}','{6}')".format(
...
def protected():
    ...
    return render_template('profile.html', name=flask_login.current_user.id, message="Here's your profile",
                           Photos=all_photos, base64=base64)
...
def create_albums():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        print(cursor.execute(
            "INSERT INTO Albums(Album_name,Date_of_creation, user_id) VALUES ('{0}','{1}','{2}')".format(Album_name,
                                                                                                         date, uid)))
        conn.commit()
...
def view_photo(photo_id):
    ...
    # show comment
    cursor.execute(
        "SELECT C.text, U.First_name FROM Comments C, Users U WHERE C.user_id = U.user_id AND C.picture_id = '{0}'".format(
            photo_id)
    )
...
def add_comment(photo_id):
    ...
    if current_user.is_authenticated:
        ...
        cursor.execute(
            "INSERT INTO Comments(text, Date, picture_id,user_id) VALUES ('{0}','{1}','{2}','{3}')".format(comment,
                                                                                                           date,
                                                                                                           photo_id,
                                                                                                           uid))
    else:
...
def friend():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        print(cursor.execute(
            "INSERT INTO Friends(Friends_id, Friends_email,user_email, user_id) VALUES ('{0}','{1}','{2}','{3}')".format(
...
def search_tags():
    ...
    if request.method == "POST":
        ...
        for tag in tags:
            cursor = conn.cursor()
            query = (
                "SELECT P.picture_id, P.imgdata, P.caption FROM Pictures P,Tags_and_pics WHERE Tags_and_pics.tag_name='{0}' and "
...
def search_own_tags():
    ...
    print(uid)
    cursor = conn.cursor()
    query = "SELECT T.tag_name FROM Tags T, Tags_and_pics TP, Pictures P WHERE T.tag_name=TP.tag_name AND TP.picture_id=P.picture_id AND P.user_id='{0}'".format(
        uid)
    cursor.execute(query)
    all_tags = cursor.fetchall()
    if request.method == "POST":
...
def hello():
    ...
    return render_template('hello.html', message='Welecome to Photoshare', Photos=all_photos, base64=base64,
                           all_albums=all_albums, email=email)
